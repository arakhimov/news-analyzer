...
function getDays() {
  ...
  const daysBeforeToday = 6; //неделя считается с текущим днем - поэтому рассчитываем 6 предидущих дней
  for (let i = 0; i <= daysBeforeToday; i++) {
    elementsDOM.daysWrapper.append(elementsDOM.daysTemplate.content.cloneNode(true));
  }
  let counter = date.getDate() - daysBeforeToday;
  // проставляем даты
...
const rangeValue = statistics.getMentionAmount();
